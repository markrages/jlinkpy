...
class Memory:
    ...
    def loadSRec(self, file):
        """load data from a (opened) file in S19 format"""
        startAddr   = 0
        segmentdata = []
        ...
        for line in file:       #Read one line
            ...
            l = line.strip()
            if l[0] != 'S': raise FileFormatError('Not S-record file')
            if l[1] == '0': pass # manufacturer-specific
            elif l[1] in ['1','2','3']:
                addrwidth = {'1':2,'2':3,'3':4}[l[1]]
                newStartAddr = int(l[4:4+2*addrwidth],16)

                #print "new segment?",startAddr,"+",len(segmentdata),newStartAddr
                if newStartAddr != startAddr + len(segmentdata):
                    #print "ok"
                ...
                x=4+addrwidth*2
                for c in range(count-addrwidth-1):
                    #print count,addrwidth,c
                    segmentdata.append(chr(int(l[x+c*2:x+c*2+2],16)))
            
            elif l[1] in ['7','8','9']:
                pass # end of block
            else:
                raise Exception('S%s unimplemented'%l[1])
